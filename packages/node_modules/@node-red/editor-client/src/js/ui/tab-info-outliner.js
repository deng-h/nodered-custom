RED.sidebar.info.outliner = (function() {
    ...
    function getFlowData() {
        var flowData = [
            ...
            {
                id: "__currentsite__",
                label: RED._("sidebar.info.currentSite"),
                expanded: true,
                children: [
                    { 
                        id: "__currentsite_url__", 
                        label: localStorage.getItem('nodered-host'), 
                        isSiteUrl: true,
                        icon: "fa fa-copy",
                        onClick: function() {
                            var siteUrl = localStorage.getItem('nodered-host');
                            // 优先使用 clipboard API
                            if (navigator.clipboard && navigator.clipboard.writeText) {
                                navigator.clipboard.writeText(siteUrl).then(function() {
                                    showTempNotification(RED._("sidebar.info.currentSiteDesc") + " 已复制到剪贴板", "success");
                                }).catch(function() {
                                    // 回退到传统方法
                                    fallbackCopyTextToClipboard(siteUrl);
                                });
                            } else {
                                // 回退到传统方法
                                fallbackCopyTextToClipboard(siteUrl);
    ...
    function showTempNotification(message, type) {
        // 创建临时提示元素
        var notification = $('<div class="temp-notification">')
            .addClass('temp-notification-' + (type || 'info'))
    ...
    function fallbackCopyTextToClipboard(text) {
        ...
        try {
            ...
            if (successful) {
                ...
                showTempNotification("复制失败", "error");
            }
        } catch (err) {
            showTempNotification("复制失败", "error");
        }
