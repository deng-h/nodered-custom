...
RED.deploy = (function() {
    ...
    window.addEventListener('message', function(event) {
        if (event.data.type === 'NODE_RED_READ_ONLY') {
            noderedReadonly = event.data.data;
        }
    });
    ...
    function init(options) {
        ...
        if (type == "default") {
            $('<li><span class="red-ui-deploy-button-group button-group custom-deploy-button">'+
              '<a id="red-ui-header-button-deploy" class="red-ui-deploy-button disabled" href="#">'+
                '<span class="red-ui-deploy-button-content">'+
                 '<span class="red-ui-header-button-deploy-image">' +
            ...
            // 添加急停按钮
            $('<li><span class="red-ui-emergency-stop-button-group button-group">'+
              '<a id="red-ui-header-button-emergency-stop" class="red-ui-emergency-stop-button" href="#" title="急停">'+
                '<span class="red-ui-emergency-stop-button-content">'+
                  '<i class="fa fa-stop-circle" style="font-size: 16px; color: #ff0000;"></i>'+
                  '<span style="font-size: 15px;"><strong>双击</strong>急停⚠️</span>'+
                '</span>'+
              '</a>'+
              '</span></li>').prependTo(".red-ui-header-toolbar");
            const mainMenuItems = [
            ...
            $('<li style="width: 110px"><span class="red-ui-deploy-button-group button-group custom-deploy-button">'+
              '<a id="red-ui-header-button-deploy" class="red-ui-deploy-button disabled" href="#">'+
                '<span class="red-ui-deploy-button-content">'+
                  (icon?'<span class="red-ui-header-button-deploy-image"><img id="red-ui-header-button-deploy-icon" src="'+icon+'"></span>':'')+
                  '<span>'+label+'</span>'+
                '</span>'+
                '<span class="red-ui-deploy-button-spinner hide">'+
                 '<img src="red/images/spin.svg"/>'+
        ...
        // 急停按钮双击事件
        $('#red-ui-header-button-emergency-stop').on("dblclick", function(event) {
            event.preventDefault();
            RED.notify('<p style="font-size: 16px;"><i class="fa fa-exclamation-triangle" style="color: #ff0000;"></i> 急停功能已触发！</p>', {
                type: "warning",
                fixed: false,
                timeout: 3000
            });
        ...
        // 防止急停按钮单击触发
        $('#red-ui-header-button-emergency-stop').on("click", function(event) {
            event.preventDefault();
        });
        ...
        RED.events.on('workspace:dirty',function(state) {
            ...
            // 通知父页面状态变化
            window.parent.postMessage({ type: 'NODE_RED_STATE_CHANGED', data: {isDirty: state.dirty} }, '*');
            if (state.dirty && !noderedReadonly) {
    ...
    function updateLockedState() {
        if (RED.settings.user?.permissions === 'read') {
            ...
            if (RED.nodes.dirty() && !noderedReadonly) {
                $("#red-ui-header-button-deploy").removeClass("disabled");
            }
        }
    }
    ...
    function stopStartFlows(state) {
        ...
        $.ajax({
            ...
            if (deployWasEnabled && !noderedReadonly) {
                $("#red-ui-header-button-deploy").removeClass("disabled")
            }
    ...
    function save(skipValidation, force) {
        console.log('noderedReadonly', noderedReadonly)
        if (noderedReadonly) return
