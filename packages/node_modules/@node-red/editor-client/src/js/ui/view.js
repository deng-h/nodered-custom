            ...
            if (hasInternal) {
                ...
                try {
                    ...
                    if (text && text.trim() !== "") {
                        ...
                        const isValid = nodes.every(n => typeof n === "object" && n.type && n.id);
                        if (isValid) {
                            // 使用外部数据
                            importNodes(nodes, { generateIds: true });
                            // 清空外部剪贴板
                            clearClipboard();
                        }
                    }
                } catch (e) {
                    console.warn("无法读取外部剪贴板内容:", e);
                }
            }
        });
