    ...
    function createTabs(options) {
        ...
        function activateTab(link) {
            ...
            if (!link.parent().hasClass("active")) {
                // Remove avatar from previously active tab
                ul.find('.red-ui-tab-user-avatar').remove();
                ...
                // Add user avatar to active tab
                var activeTab = link.parent();
                // Only add avatar for active tabs within red-ui-workspace-tabs
                if (activeTab.closest('#red-ui-workspace-tabs').length > 0) {
                    var avatarContainer = $('<div class="red-ui-tab-user-avatar"><img src="red/images/hos-avator.svg" /></div>').appendTo(activeTab);
                    var currentMenu = null;
                    var isHovering = false;

                    avatarContainer.on('click', function(evt) {
                        evt.preventDefault();
                        evt.stopPropagation();
                        return;
                    });
                    ...
                    avatarContainer.on('mouseenter', function(evt) {
                        isHovering = true;
                        showMenu();
                    ...
                    function showMenu() {
                        ...
                        var elementPos = avatarContainer.offset();
                        currentMenu.css({
                            position: "absolute",
                            top: (elementPos.top + avatarContainer.height() + 5) + "px",
                            left: (elementPos.left - currentMenu.width() + avatarContainer.width()) + "px"
                        });
                        
                        currentMenu.appendTo("body");
                        menuOptions.length > 0 && currentMenu.show();
                    }

                    // Listen for user data from parent window
                    window.addEventListener('message', function(event) {
                        if (event.data && event.data.type === 'NODE_RED_OPERATING_USERS') {
                            var userData = event.data.data;
                            if (userData && Array.isArray(userData)) {
                                // Create menu options from user data
                                menuOptions = userData.map(function(user) {
                                    return {
                                        label: user.userName,
                                        disabled: false,
                                        onselect: function(){}
                                ...
                                // If currently hovering, update the menu
                                if (isHovering) {
                                    showMenu();
                                }
                            }
