...
var RED = (function() {
    ...
    function buildEditor(options) {
        ...
        $('<div id="red-ui-main-container" class="red-ui-sidebar-closed hide">'+
            '<div id="red-ui-workspace"><ul class="red-ui-header-toolbar hide"></ul></div>'+
            '<div id="red-ui-editor-stack" tabindex="-1"></div>'+
            '<div id="red-ui-palette"><div id="hos-btns"><a id="hos-add" title="添加技能"><img src="red/images/hos-add.svg" alt="添加技能" /></a><a id="hos-import" title="导入流程"><img src="red/images/hos-import.svg" alt="导入流程"></a><a id="hos-download" title="下载部署"><img src="red/images/hos-download.svg" alt="下载部署"></a></div></div>'+
        ...
        // 根据父组件的postmessage判断是否显示hos-btns
        window.addEventListener('message', function(event) {
            if (event.data.type === 'NODE_RED_MODE') {
                const showHosBtns = event.data.data === "cloud";
                if (showHosBtns) {
                    $("#hos-btns").show();
                    $(".red-ui-palette-scroll").css("bottom", '85px');
                } else {
                    $("#hos-btns").hide();
                    $(".red-ui-palette-scroll").css("bottom", '25px');
                }
            }
        });

        // 根据父组件的是否只读判断hos-btns是否只读
        window.addEventListener('message', function(event) {
            if (event.data.type === 'NODE_RED_READ_ONLY') {
                const showHosBtns = !event.data.data;
                if (showHosBtns) {
                    $("#hos-btns").show();
                    $(".red-ui-palette-scroll").css("bottom", '85px');
                } else {
        ...
        // 点击添加技能
        $('#hos-add').on("click", function(event) {
            event.preventDefault();
            window.parent.postMessage({ type: 'HOS_ADD_SKILL'}, '*');
        });

        // 点击下载
        $('#hos-download').on("click", function(event) {
            event.preventDefault();
            if (RED.nodes.dirty()) {
                RED.notify(RED._("notification.warnings.undeployedChanges"),"warning");
                return;
            }
            const flows = RED.nodes.createCompleteNodeSet();
            window.parent.postMessage({ type: 'HOS_DOWNLOAD', data: {
                flows
            }}, '*');
        });

        // 点击导入
        $('#hos-import').on("click", function(event) {
            event.preventDefault();
            window.parent.postMessage({ type: 'HOS_IMPORT_FLOW'}, '*');
        });

        // 监听页面点击回退
        window.addEventListener('popstate', () => {
            if (window._nodered_internal_nav) {
                window._nodered_internal_nav = false;
                return;
            }
            window.parent.postMessage({ type: 'HOS_POPSTATE' }, '*');
        });
    }
